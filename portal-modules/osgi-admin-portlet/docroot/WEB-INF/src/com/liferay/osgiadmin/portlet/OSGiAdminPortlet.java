...
import com.liferay.portal.module.framework.MFException;
import com.liferay.portal.module.framework.MFUtil;
		...
		try {
			...
			if (cmd.equals("install-from-upload")) {
				...
				if ((file == null) || !file.exists()) {
					throw new MFException("file-does-not-exist");
				}

				MFUtil.addBundle(location, new FileInputStream(file));
			}
			else if (cmd.equals("install-from-remote-location")) {
				MFUtil.addBundle(location);
			}
				...
				MFUtil.updateBundle(bundleId, new FileInputStream(file));
			}
			else if (cmd.equals("update-from-remote-location")) {
				long bundleId = ParamUtil.getLong(uploadRequest, "bundleId");

				MFUtil.updateBundle(bundleId);
			}
			else if (cmd.equals("uninstall")) {
				long bundleId = ParamUtil.getLong(uploadRequest, "bundleId");

				MFUtil.uninstallBundle(bundleId);
			}
