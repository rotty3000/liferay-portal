...
package com.liferay.remote.app.config.factory;
...
import com.liferay.remote.app.config.factory.configuration.v1.RemoteAppConfiguration;
 ...
 */
@Component(
	configurationPid = "com.liferay.remote.app.config.factory.configuration.v1.RemoteAppConfiguration",
	configurationPolicy = ConfigurationPolicy.REQUIRE,
	immediate = true
)
public class RemoteAppConfigFactory {
	...
	public RemoteAppConfigFactory(
		...
		RemoteAppConfiguration remoteAppConfiguration =
		...
		Map<Locale, String> nameMap = Collections.singletonMap(
			LocaleUtil.getMostRelevantLocale(), remoteAppConfiguration.name());

		RemoteAppEntry remoteAppEntry =
			_remoteAppEntryLocalService.fetchRemoteAppEntryByExternalReferenceCode(
				company.getCompanyId(), externalReferenceCode);

		if (remoteAppEntry == null) {
			remoteAppEntry = _remoteAppEntryLocalService.addOrUpdateCustomElementRemoteAppEntry(
				externalReferenceCode, defaultAdminUser.getUserId(),
				StringUtil.merge(remoteAppConfiguration.webComponentCssUrl(), ","),
				remoteAppConfiguration.elementName(),
				StringUtil.merge(remoteAppConfiguration.webComponentUrl(), ","), false,
				remoteAppConfiguration.description(),
				remoteAppConfiguration.friendlyURLMapping(),
				remoteAppConfiguration.instanceable(), nameMap,
				remoteAppConfiguration.portletDisplayCategory(),
				remoteAppConfiguration.portletServiceProperties(), null);

			_remoteAppEntryLocalService.updateStatus(
				defaultAdminUser.getUserId(), remoteAppEntry.getRemoteAppEntryId(),
