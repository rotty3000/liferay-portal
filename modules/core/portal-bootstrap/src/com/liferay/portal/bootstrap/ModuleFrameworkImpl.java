...
public class ModuleFrameworkImpl implements ModuleFramework {
	...
	private void _configureModuleDirs() {
		File file = new File(
			PropsValues.MODULE_FRAMEWORK_PORTAL_DIR,
			"org.apache.felix.fileinstall-modules.cfg");

		if (PropsValues.MODULE_FRAMEWORK_AUTO_DEPLOY_DIRS.length < 1) {
			if (file.exists()) {
				file.delete();
			}
		...
		properties.put(
			FrameworkPropsKeys.FELIX_FILEINSTALL_ACTIVE_LEVEL,
			String.valueOf(PropsValues.MODULE_FRAMEWORK_BEGINNING_START_LEVEL));
		properties.put(
			FrameworkPropsKeys.FELIX_FILEINSTALL_DIR,
		...
		properties.put(
			...
			String.valueOf(PropsValues.MODULE_FRAMEWORK_AUTO_DEPLOY_INTERVAL));
		properties.put(
			FrameworkPropsKeys.FELIX_FILEINSTALL_START_ACTIVATION_POLICY,
			Boolean.FALSE.toString());
		properties.put(
			FrameworkPropsKeys.FELIX_FILEINSTALL_START_TRANSIENT,
			Boolean.FALSE.toString());
		properties.put(
			FrameworkPropsKeys.FELIX_FILEINSTALL_TMPDIR,
			SystemProperties.get(SystemProperties.TMP_DIR));
		...
		try (OutputStream outputStream = new FileOutputStream(file)) {
			properties.store(outputStream, "Autogenerated by the portal.");
		}
		catch (IOException ioe) {
