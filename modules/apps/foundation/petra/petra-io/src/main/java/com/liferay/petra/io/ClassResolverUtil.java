/**
 * Copyright (c) 2000-present Liferay, Inc. All rights reserved.
 *
 * This library is free software; you can redistribute it and/or modify it under
 * the terms of the GNU Lesser General Public License as published by the Free
 * Software Foundation; either version 2.1 of the License, or (at your option)
 * any later version.
 *
 * This library is distributed in the hope that it will be useful, but WITHOUT
 * ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS
 * FOR A PARTICULAR PURPOSE. See the GNU Lesser General Public License for more
 * details.
 */

package com.liferay.petra.io;

import com.liferay.petra.io.internal.loader.DefaultClassResolver;
import com.liferay.petra.io.spi.loader.ClassResolver;

import java.util.Iterator;
import java.util.ServiceLoader;

/**
 * @author Raymond Augé
 */
public class ClassResolverUtil {

	public static Class<?> resolve(String className, ClassLoader classLoader)
		throws ClassNotFoundException {

		return _classResolver.resolve(className, classLoader);
	}

	public static Class<?> resolveByContextClassLoader(String className) {
		return _classResolver.resolveByContextClassLoader(className);
	}

	public static Class<?> resolveByPortalClassLoader(String className) {
		return _classResolver.resolveByPortalClassLoader(className);
	}

	public static Class<?> resolveByPortletClassLoader(
		String className, String servletContextName) {

		return _classResolver.resolveByPortletClassLoader(
			className, servletContextName);
	}

	private ClassResolverUtil() {

		// Prevent Instantiation

	}

	private static final ClassResolver _classResolver;

	static {

		// Deliberately avoid using thread context class loader

		ServiceLoader<ClassResolver> serviceLoader = ServiceLoader.load(
			ClassResolver.class, ClassResolver.class.getClassLoader());

		Iterator<ClassResolver> iterator = serviceLoader.iterator();

		if (iterator.hasNext()) {
			_classResolver = serviceLoader.iterator().next();
		}
		else {
			_classResolver = new DefaultClassResolver();
		}
	}

}