...
public class DestinationStatisticsTest extends TestUtil {
	...
	protected void test(String bundle, String destinationName)
		...
		try {
			...
			assertStats("Before Stats %s:%n", destinationName, destination.getDestinationStatistics());
			...
			assertStats("Updated Stats %s:%n", destinationName, destination.getDestinationStatistics());

			callable.call();

			Thread.sleep(1000);

			assertStats("Final Stats %s:%n", destinationName, destination.getDestinationStatistics());
		}
		finally {
			tbBundle.uninstall();
		}
