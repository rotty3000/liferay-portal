...
package com.liferay.page.comments.web.action;
...
@Component(
		immediate = true,
		property = {
			"action.command.name=edit_page_discussion",
			"javax.portlet.name=com_liferay_page_comments_web_portlet_PageCommentsPortlet"
		},
		service = ActionCommand.class
	)
public class EditDiscussionAction extends BaseActionCommand {
	...
	protected void doProcessCommand(
		...
		try {
			...
			if (cmd.equals(Constants.ADD) || cmd.equals(Constants.UPDATE)) {
				MBMessage message = updateMessage(portletRequest);

				boolean ajax = ParamUtil.getBoolean(portletRequest, "ajax");

				if (ajax) {
					String randomNamespace = ParamUtil.getString(
						portletRequest, "randomNamespace");
					...
					writeJSON(portletRequest, portletResponse, jsonObject);

					return;
				}
			}
			else if (cmd.equals(Constants.DELETE)) {
				deleteMessage(portletRequest);
			}
			else if (cmd.equals(Constants.SUBSCRIBE_TO_COMMENTS)) {
				subscribeToComments(portletRequest, true);
			}
			else if (cmd.equals(Constants.UNSUBSCRIBE_FROM_COMMENTS)) {
				subscribeToComments(portletRequest, false);
			}

			sendRedirect(portletRequest, portletResponse, redirect);
		}
		catch (Exception e) {
	...
	private void sendRedirect(PortletRequest portletRequest,
			PortletResponse portletResponse, String redirect) {
		System.out.println("\n\n\n\n--------------------------------> redirect::" + redirect);
	}
	...
	**/
	...
	protected void deleteMessage(PortletRequest actionRequest) throws Exception {
	...
	protected void subscribeToComments(
			PortletRequest actionRequest, boolean subscribe)
		throws Exception {
	...
	protected MBMessage updateMessage(PortletRequest actionRequest)
		throws Exception {
