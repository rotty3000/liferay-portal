...
import static com.liferay.portal.upgrade.task.UpgradeTask.create;
...
public class UpgradeProcess_7_0_0 extends UpgradeProcess {
	...
	protected void doUpgrade() throws Exception {
		UpgradeProcessExecutor upgradeProcessExecutor =
			new GuavaFutureExecutor(Executors.newCachedThreadPool());

		UpgradeTask upgradeSchemaTask = create(new UpgradeSchema());

		upgradeProcessExecutor.schedule(upgradeSchemaTask);

		UpgradeTask upgradeAddressTask = UpgradeTask.create(
			new UpgradeAddress());

		upgradeProcessExecutor.schedule(upgradeAddressTask, upgradeSchemaTask);
